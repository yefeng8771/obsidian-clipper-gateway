...
AI 客户端 (MCP)        ─SSE  /api/mcp/sse─┤
                                 ...
                                 └────────┬───────┘
                                          │ REST  ${FNS_HOST} (如 https://fns.oba.plus)
                                 ┌────────▼─────────────────────────┐
...
> **注意**：如果 `FNS_HOST` 已配置（如 `https://fns.oba.plus`），web_clipper 会直接走对外域名通信，此时不再依赖 `internal` 网络连接 fast-note-sync-service。

## 鉴权

- **`API_KEY`**：客户端（浏览器扩展、curl）→ web_clipper 的 Bearer
